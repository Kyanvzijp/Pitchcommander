...
# ----- Camera -----
# Lensmodel voor de kalibratie/correctie:
#   "fisheye"  : de huidige OV5647 nachtzicht-fisheye
#   "standard" : de Global Shutter camera met de 6 mm CS-mount lens
LENS_MODEL = "standard"

# ============ ACTIEF PROFIEL: HQ Camera (IMX477) + 6 mm lens ============
# Modus 1332x990 is 2x2 gebinned (4x licht per pixel) en haalt 120 fps.
# Beeldveld op 2 m: ~1,38 x 1,02 m; de bal is daar ~70 px breed.
# Terugvalprofiel OV5647 (oude nachtzichtcamera):
#   CAM_WIDTH=640, CAM_HEIGHT=480, CAM_FPS=58, CAM_EXPOSURE_US=6000,
#   CAM_GAIN=8.0, LENS_MODEL="fisheye", MIN_BLOB_AREA=30,
#   MAX_BLOB_AREA=2200
# ========================================================================
...
# LET OP: schaalt mee met resolutie en framerate. Op dit HQ-profiel
# (1332 px breed, 120 fps, ~0,96 px/mm op 2 m): zachte worp 3 m/s ~ 24
# px/frame, hand ~ 8-12. Op het oude OV5647-profiel was 10 de juiste
# waarde.
...
TRACK_MAX_LEN = 80
# Een bal is 1 blob (hooguit 2 met schaduw); een arm of persoon valt
# uiteen in VEEL blobs tegelijk. Boven dit aantal gelijktijdige blobs
# gaat de impactbepaling op slot: er beweegt dan een persoon in beeld.
MAX_SIMULTANEOUS_BLOBS = 3
# Nadat het druk was blijft de poort nog dit aantal frames dicht, zodat
# een wegtrekkende arm (die kort onder de blobdrempel zakt) geen valse
# baan kan starten. ~0,2-0,3 s; een worp direct erna blijft mogelijk.
BUSY_HOLD_FRAMES = 25  # ~0,2 s op 120 fps
# Aantal frames dat een blob "rust" moet hebben voordat we een nieuwe
# impact accepteren (debounce, voorkomt dubbele detectie).
IMPACT_COOLDOWN_FRAMES = 50   # ~0,4 s op 120 fps
...
SUPPRESS_AFTER_DRAW_FRAMES = 30  # ~0,25 s op 120 fps
